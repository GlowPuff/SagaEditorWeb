...
export default function CampaignRewardTab4({
  items,
  changeItems,
  selectedTabIndex,
  tabIndex,
}) {
  const [selectedItem, setSelectedItem] = useState(allyData[0]); //{id, name});
        ...
        >
          {items.map((item, index) => (
            <div key={index} className="event-container align-center">
              <Typography>
                {allyData.find((x) => x.id === item).name} /{" "}
                {allyData.find((x) => x.id === item).id}
              </Typography>
