...
//data
import { rewardData } from "../../data/carddata";

export default function CampaignRewardTab2({
  items,
  changeItems,
  selectedTabIndex,
  tabIndex,
}) {
  const [selectedItem, setSelectedItem] = useState(rewardData[0]); //{id, name});
        ...
        >
          {items.map((item, index) => (
            <div key={index} className="event-container align-center">
              <Typography>
                {rewardData.find((x) => x.id === item).name} /{" "}
                {rewardData.find((x) => x.id === item).id}
              </Typography>
